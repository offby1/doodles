...
(define *the-environment* (environment-make))

(define (my-eval exp)
  (internal-eval exp *the-environment*))
...
;; Now add internal-eval and my-apply, but under the normal names.
(set! snarfed-procedures (cons (cons 'apply my-apply)
                               (cons (cons 'eval internal-eval)
                                     snarfed-procedures)))

