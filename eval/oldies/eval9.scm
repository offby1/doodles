...
(define (are-good-frame-arguments? names values)
  (and (eq? 'list (is-formals-list? names))
       ...
       ((eq? head 'define)
        ...
        (if (symbol? (car args))
            (begin
              ...
              (let ((binding (frame-lookup (environment-current-frame env) (car args)))
