...
(define (make-maze width-in-cells)
  ...
  (define (grid-render-in-postscript solution)
    (let ((array-width (car (array-dimensions grid)))
      ...
      (let ((scale (/ (* 6 72) array-width)))
        (define (general-line x0 y0 x1 y1)
          (string-append
           (number->string (exact->inexact x0))
           " "
        ...
        (define usual-prelude
          (string-append
           "%!PS-Adobe-3.0\n%%Title: Maze\n%%LanguageLevel: 2\n%%EndComments\n"
           "18 18 translate\n"
           (number->string (exact->inexact scale))
           " "
           (number->string (exact->inexact scale))
           " scale\n"
           "1 "
        ...
        (define (render-path path)
          (if (and
              ...
              (let ((here (car path))
                ...
                (string-append (general-line (point-x here)
                                             (point-y here)
                                             (point-x there)
                                             (point-y there))
                               (render-path (cdr path))))
            ""))
...
(let ((temp-file-name (tmpnam)))
  ...
  (call-with-output-file temp-file-name (lambda (port) (display (make-maze 10) port)))
