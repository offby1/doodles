...
(define (make-maze width-in-cells)
  ...
  (define (grid-render-in-postscript)
    (let* ((array-width (car (array-dimensions grid)))
      ...
      (define (dot x y radius)
        ;; "arc" in PostScript takes five arguments: center x, center
        ;; y, radius, start-angle (in degrees), stop angle.
        (string-append
         (move-to (+ x (/ radius 2)) y)
         (number->string (exact->inexact x))
         " "
         (number->string (exact->inexact y))
         " "
      ...
      (define (arrow x y direction)
        ...
        (let ((endx x)
          ...
          (case direction
            ((north) (set! endy (+ y length)))
            ((south) (set! endy (- y length)))
            ((east)  (set! endx (+ x length)))
            ((west)  (set! endx (- x length)))
            (else (error "Uh oh")))
          (string-append
           (general-line x y endx endy)
           (dot endx endy (/ length 10)))))
                               ...
                               (string-append
                                ...
                                (if (or (memq 'south cell)
                                  ...
                                  (horizontal-line columns-processed
                                                   rows-processed
                                                   1))
                                (let ((where-to-go-from-here (member (cons columns-processed rows-processed) (reverse solution))))
                                  (if (and
                                       (> (length cell)
                                          2)
                                      ...
                                      (arrow (+ 1/2 columns-processed)
                                             (+ 1/2 rows-processed)
                                             (direction-indicator (cadr where-to-go-from-here)
                                                                  (car where-to-go-from-here)))
                                    ""))
                                )
                               )))))))))
       page-coda
       (page-prelude "The solution")
  ...
  (wander-from! (make-point
                 ...
                 ;; (quotient (cadr (array-dimensions grid)) 2)
                 ) #f)
