    ...
    (let* ((array-width (car (array-dimensions grid)))
      ...
      (define (arrow x y direction)
        ...
        (let ((angle (case direction
          ...
          (string-append
           "gsave 0.5 setgray\n"
           (number->string (exact->inexact (/ length 10)))
           " setlinewidth\n"
           (number->string (exact->inexact x))
           " "
           (number->string (exact->inexact y))
           ...
           " rotate\n"
           (number->string (exact->inexact (- (/ length 2))))
           " 0 translate\n"
           (move-to 0 0)
           (draw-to length 0)
       ...
       (let ((entrance (car solution))
         ...
         (define (with-origin p . stuff)
           (string-append
            ...
            " translate\n"
            (apply string-append stuff)
            "grestore\n"))
         (define (rotate angle)
           (string-append (number->string (exact->inexact angle))
                          " rotate\n"))
         (string-append
          (with-origin
           entrance
