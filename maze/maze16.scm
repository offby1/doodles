...
(define uneven-random
  (let ((x -1))
    (lambda (upper)
      (define (squish x) (* x x x x))
      (floor (* upper (squish (/ (random most-positive-fixnum) most-positive-fixnum))))
      ;; (set! x (+ 1 x))
      ;; (remainder x upper)
      ))
  )
...
(define (make-maze grid-width)
  ...
  (define grid-height (inexact->exact (round (* 5/4 grid-width))))
