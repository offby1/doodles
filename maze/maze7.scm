    ...
    (let* ((array-width (car (array-dimensions grid)))
      ...
      (define (arrow x y direction)
        ...
        (let ((angle (case direction
          ...
          (string-append
           "gsave\n"
           (number->string (exact->inexact (/ length 5)))
           " setlinewidth\n"
           (number->string (exact->inexact x))
           " "
           (number->string (exact->inexact y))
           ...
           (move-to 0 0)
           (draw-to length 0)
           (move-to length 0)           ;seems redundant, but avoids
                                        ;an ugly pointy mark on the
                                        ;arrow.  Setting the
                                        ;line-ending style is the
                                        ;proper way to take care of
                            ...
                            (string-append
                             ...
                             (let ((cell (array-ref grid columns-processed rows-processed))
                               ...
                               (string-append
                                ...
                                (let ((path-to-exit (member (cons columns-processed rows-processed) (reverse solution))))
                                  (if (and
                                       (> (length cell)
                                          2)
                                       path-to-exit)
