...
;; find funny quotes from jordanb in my IRC logs, and write them to
;; standard output.  Note that this all depends utterly on the format
;; of the log files; in my case, they're generated by the "ERC" client
...
(require (only-in (lib "etc.ss") this-expression-source-directory)
         ...
         (only-in (lib "1.ss" "srfi")
                  ...
                  take
                  third)
         (only-in (planet "main.ss" ("dherman" "memoize.plt" )) define/memo*)
         (only-in (planet "port-to-lines.ss" ("offby1" "offby1.plt"))
                  file->lines
                  )
