...
(define hpane
  (instantiate
   (class horizontal-pane% (super-new))
   ()
   (parent frame)))

(define csv-panel
  (instantiate
   (class panel% (super-new))
   ()
   (parent hpane)
   (style '(border))))
...
(define open-button
  (instantiate
   (class button% (super-new))
   ()
   (parent csv-panel)
   (label "Read dem CSV files")
...
(define download-button
  (instantiate
   ...
   (label "Snarf photo data from flickr")
   (callback
    (lambda (item event)
...
(define join-button
 (instantiate
  ...
  ()
  (parent joined-panel)
  (label "glue the two bits together")
  (callback
   (lambda (item event)
     (let ((joined
            (filter
